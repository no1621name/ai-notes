...
export const removeTagFromNote = (dataTransfer: DBDataTransfer, noteId: PrimaryKeyType, tagId: PrimaryKeyType) => {
  const relationManager = new ManyToManyManager(noteToTagRelationConfig, dataTransfer);
  return relationManager.deleteRelation(noteId, tagId);
};
